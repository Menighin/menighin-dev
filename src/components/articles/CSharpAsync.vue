...
export default {
    ...
    data() {
        return {
            title: {
                'pt-br': 'Async / Await: Primeiros passos',
                'en-us': 'Async / Await: First steps',
            },
            route: 'c-sharp-async',
            previewImg: '/imgs/csharpsyncasync/preview.png',
            previewText: {
                'pt-br': 'O que são métodos async? Eles rodam em outra thread? Como async pode fazer meu código ficar mais performático? Vamos responder essas e outras perguntas com exemplos.',
                'en-us': 'What are async methods? Do they run in another thread? How can async make my code run faster? Let\'s answer these and another questions with examples.'
            },
            ...
            content: [
            {
                type: 'p',
                text: {
                    'pt-br': `Esse vai ser um artigo rápido e simples sobre como usar o <code>async</code> pra fazer o seu método rodar em outra thread e mais rápido.
                    Vamos lá, mãos à obra com o primeiro exemplo: `,
                    'en-us': `This is going to be a quick and simple article on how to use <code>async</code> to make your method run in another thread and faster. Let's 
                    start with our first example th'en-us': `
                }
            },
            {
                type: 'code',
    ...
    public static async Task HeavyWorkAsync()
    {
        ...
        Console.WriteLine($"[T{Thread.CurrentThread.ManagedThreadId}] Heavy work finished");
            ...
            },
            {
                type: 'p',
                text: {
                    'pt-br': `No exemplo acima, temos o método <code>Main</code> chamando o método assíncrono <code>HeavyWorkAsync</code> que simula uma função
                    ...
                    'en-us': `In the above example, we have the <code>Main</code> method calling the asynchronous <code>HeavyWorkAsync</code> that simulates the execution of 
...
[T1] Ended main after 10ms`
            },
            {
                type: 'p',
                text: {
                    'pt-br': `Conforme esperado, o método assíncrono foi executado em outra thr... Espera um pouco... Algo está errado. Aparantemente o método 
                    ...
                    'en-us': `As expected, the asynchronous method was executed in anot... Wait a sec... Something is wrong. Apparently the method was executed on the same thread...? 
            ...
            },
            {
                type: 'p',
                text: {
                    'pt-br': `Pois é, tenho certeza que não fui o primeiro e nem terei sido o último que, ao ver a palavra <code>async</code> na declaração
                    ...
                    'en-us': `Yes, I'm sure I wasn't the first nor will be the last that, seeing the <code>async</code> word in a method's signature, assumed it would be executed 
            ...
            },
            {
                type: 'h3',
                text: {
                    'pt-br': `Assíncronismo X Multithreading`,
                    'en-us': `Asynchronous X Multithreading`
                }
            },
            {
                type: 'p',
                text: {
                    'pt-br': `Para melhor entender a diferença entre assíncronismo e multithreading vamos utilizar um exemplo. Você acorda pela manhã e precisa preparar o seu café
                    ...
                    'en-us': `To better understand the differences between asynchronous and multithreading let's use an example. You wake up in the morning and need to prepare your
            ...
            },
            {
                type: 'quote',
                text: {
                    'pt-br': `<strong>Multithreading</strong> está relacionado com <strong>Workers</strong>. <strong>Assíncronismo</strong> esta relacionado à <strong>Tasks</strong>.`,
                    'en-us': `<strong>Multithreading</strong> is related to <strong>Workers</strong>. <strong>Asynchronism</strong> is related to <strong>Tasks</strong>.`,
                }
            },
            {
                type: 'p',
                text: {
                    'pt-br': `No nosso exemplo, tasks são as diferentes tarefas (preparar o café, preparar o pão, assistir a TV) e workers são as pessoas envolvidas (nossos pais e nós mesmos).`,
                    'en-us': `In our example, we have different tasks (prepare the coffee, prepare the toast, watch tv) and workers are the people involved (our parents and ourselves).`
                }
            },
            {
                type: 'h3',
                text: {
                    'pt-br': `Mesma thread, tasks diferentes?`,
                    'en-us': `Same thread, different tasks?`
                }
            },
            {
                type: 'p',
                text: {
                    'pt-br': `Para o leitor menos acostumado com os conceitos acima tudo pode parecer meio estranho. Afinal, aprendemos na faculdade que um método executa de forma sequencial,
                    linha após linha. Então como é possível uma única thread executar diferentes tasks ao mesmo tempo?`,
                    'en-us': `To the reader less used to the above concepts, it all can seem a bit odd. After all, we usually learn that a method runs sequentially, line after line.
            ...
            },
            {
                type: 'p',
                text: {
                    'pt-br': `A realidade é que uma mesma thread não consegue executar diferentes tasks ao mesmo tempo (ênfase no executar). Basicamente, tasks processor-bound (que dependendem do
                    ...
                    'en-us': `The truth is a thread cannot execute different tasks at the same time (emphasis on "execute"). Basically, tasks processor-bound (that depend on the processor) are executed
            ...
            },
            {
                type: 'p',
                text: {
                    'pt-br': `Trazendo o nosso café da manhã de volta para o mundo da programação, fazer o café e as torradas seriam tarefas que não dependem do nosso processador,
                    ...
                    'en-us': `Bringing our breakfast back to the programming world, preparing the coffee and the toast would be non processor-bound tasks, for example, a web request or an
            ...
            },
            {
                type: 'h2',
                text: {
                    'pt-br': `Async / Await no C#`,
                    'en-us': `Async / Await in C#`
                }
            },
            {
                type: 'p',
                text: {
                    'pt-br': `As keywords <code>async</code> e <code>await</code> foram introduzidas no C# 5.0 e mudaram a forma como programação assíncrona é feita na linguagem. Vamos detalhar como utilizá-las nos próximos tópicos.`,
                    'en-us': `The keywords <code>async</code> and <code>await</code> were introduced in C# 5.0 and changed the way we do asynchronous programming. Let's detail a bit how to use them on the next topics.`
                }
            },
            {
                type: 'h3',
            ...
            },
            {
                type: 'p',
                text: {
                    'pt-br': `A palavra reservada <code>async</code> é utilizada para marcar um método e indicar que o mesmo é assíncrono (retorna um <code>awaitable</code>). O uso do <code>async</code> no método habilita
                    ...
                    'en-us': `The keyword <code>async</code> is used to mark a method and indicate that the method is asynchronous (returns an <code>awaitable</code>). The use of <code>async<code> in the signature enables
            ...
            },
            {
                type: 'p',
                text: {
                    'pt-br': `Um método marcado como <code>async</code> deve ter um dos seguintes tipos de retorno:
                    <ul>
                    ...
                    'en-us': `A method marked as <code>async</code> should have one of the following return types:
                    <ul>
                        <li><code>Task</code></li>
                        <li><code>Task&ltTResult&gt</code></li>
                        <li><code>void</code></li>
                ...
                code: `public async Task<int> GetNumberOfFriendsAsync()
{
    ...
    return friends;
            ...
            },
            {
                type: 'p',
                text: {
                    'pt-br': `Um detalhe importante de se notar é que apesar do tipo de retorno ser <code>Task&lt;int&gt;</code> estamos retornando um <code>int</code> diretamente.
                    ...
                    'en-us': `An important detail to notice is, although the return type is <code>Task&lt;int&gt;</code>, we are returning an <code>int</code> explicitly. If
            ...
            },
            {
                type: 'p',
                text: {
                    'pt-br': `A palavra reservada <code>await</code> atua como um operador unário recebendo um parâmetro - um <code>awaitable</code>. Um <code>awaitable</code> é qualquer
                    ...
                    'en-us': `The keyword <code>await</code> acts as a unary operator. It receives a parameter - an <code>awaitable</code>. An <code>awaitable</code> is any asynchronous
            ...
            },
            {
                type: 'p',
                text: {
                    'pt-br': `Quando um <code>await</code> é executado em um <code>awaitable</code> que não terminou, o <code>awaitable</code> captura o contexto e, após a operação assíncrona
                    ...
                    'en-us': `When an <code>await</code> is executed in an <code>awaitable</code> that has not finished yet, the <code>awaitble</code> saves the context and, after resolving the async
            ...
            },
            {
                type: 'p',
                text: {
                    'pt-br': `Entendendo como o contexto é manuseado fica claro que assíncronismo é diferente de multi-threading. Todo o método assíncrono é executado no mesmo contexto por default e, portanto,
                    ...
                    'en-us': `By understanding how the context is managed it also makes clear that asynchronism is different from multi-threading. Every async method is executed, by default, on the same context and therefore
...
public async Task LogOnFileAsync()
{
    ...
    // From here on, we are in a different context
            ...
            },
            {
                type: 'p',
                text: {
                    'pt-br': `Muito bem, vamos fazer finalmente o nosso café da manhã assíncrono. No exemplo abaixo, utilize as setas para seguir com a explicação passo a passo.`,
                    'en-us': `Ok, so let's finally prepare our async coffee. In the below example, make use of the arrows to navigate through the code explanation.`
                }
            },
            {
                type: 'code-explain',
                code: `class Program
{
    ...
    static async Task PrepareToastAsync()
    {
        ...
        Console.WriteLine("Toast is ready!");
                ...
                explanation: [
                    {
                        'pt-br': `Acordamos e vamos até a cozinha preparar o nosso café.`,
                        'en-us': 'We wake up and head up to the kitchen to prepare our breakfast.',
                        line: 5
                    },
                    {
                        'pt-br': `Chamamos o método assíncrono para iniciar o preparo do café`,
                        'en-us': 'We call the async method to start preparing the coffee',
                        line: 8
                    },
                    {
                        'pt-br': `O método assíncrono é iniciado imediatamente. Colocamos os igredientes na cafeteira e ligamos.`,
                        'en-us': 'The async method starts executing right away. We put the ingredients in the coffee machine and turn it on.',
                        line: 24
                    },
                    {
                        'pt-br': `Aqui o nosso primeiro <code>await</code> é executado. Fazemos um <code>delay</code> na task para simular a cafeteira. 
                            O método será suspenso e o controle retornará para a função <code>Main</code>.`,
                        'en-us': `Here our first <code>await</code> is executed. We execute a <code>delay</code> in the task to simulate the coffee machine.
                            The method is suspended and the control is returned to the <code>Main</code> function.`,
                        line: 25
                    },
                    {
                        'pt-br': `Continuando no fluxo de execução, iniciamos a task da torradeira.`,
                        'en-us': 'Moving on the execution, we start the toaster task.',
                        line: 11
                    },
                    {
                        'pt-br': `Assim como a cafeteira, o método inicia imediatamente, executando o nosso <code>Console.WriteLine</code> e...`,
                        'en-us': 'Just like the coffee machine, the method starts right away, executing the <code>Console.WriteLine</code> and...',
                        line: 31
                    },
                    {
                        'pt-br': `... é suspenso ao encontrar novamente um <code>await</code>. Novamente, o fluxo retorna para a <code>Main</code>`,
                        'en-us': '... gets suspended upon hitting the <code>await</code>. Again, the control is returned to the <code>Main</code>',
                        line: 32
                    },
                    {
                        'pt-br': `De volta à <code>Main</code> vamos assistir TV enquanto o café é preparado.`,
                        'en-us': 'Back in the <code>Main</code> we head to the living room to watch some TV while the coffee is being prepared.',
                        line: 14
                    },
                    {
                        'pt-br': `Enquanto assistimos TV, fazemos o <code>await</code> para esperar até que as torradas e o café estejam prontos.
                        ...
                        'en-us': `While we are watching the TV, we make an <code>await</code> to wait until the toasts and the coffee are ready.
                    ...
                    {
                        'pt-br': `Ao resolver um <code>await</code>, o fluxo de execução continua de onde havia parado. Aqui é onde o contexto é restaurado, lembrando que um Console Application
                        ...
                        'en-us': `Upon resolving one <code>await</code>, the execution flow continues from where it was halted. Here is where the context is restored. Just remember that this is a
                    ...
                    {
                        'pt-br': `O controle volta novamente para a <code>Main</code>. Aqui, a execução continua esperando pois o <code>Task.WhenAll()</code> só é resolvido quando todos os seus 
                        <code>awaitable</code>s são resolvidos.`,
                        'en-us': `The control is handed back to the <code>Main</code>. Here, the execution keeps waiting because <code>Task.WhenAll()</code> is only fully resolved when all of its
                        <code>awaitables</code> are resolved.`,
                        line: 17
                    },
                    {
                        'pt-br': `Novamente, ao resolver o <code>await</code> da torradeira, o método termina de executar.`,
                        'en-us': `Again, upon resolved the <code>await</code> of the toaster, the method finishes executing.`,
                        line: 33
                    },
                    {
                        'pt-br': `Finalmente, nosso café está pronto e podemos comer tranquilamente. Conseguimos prepará-lo de forma assíncrona e de forma mais rápida, gastando cerca de
                        ...
                        'en-us': `Finally, our breakfast is ready to eat! We were able to prepare it in an asynchronous and faster way, spending about 3 seconds to do so (that's some good coffee machine).
